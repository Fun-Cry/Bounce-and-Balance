...
class CoppeliaMountainEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, scene_params=None, sensor_config=None, 
                 max_episode_steps=1000, mode='normal'): # Added 'mode' parameter
        super().__init__()
        ...
        if self.sensor_config['type'] == 'camera':
            ...
            # If mode is 'joints_only' and sensor_type is 'lidar', lidar data will still be processed.
            # Consider if lidar should also be zeroed out in 'joints_only' mode.
            # For now, only 'camera' image is explicitly zeroed.
    ...
    def _get_observation(self):
        ...
        if self.sensor_config['type'] == 'camera':
            res = self.sensor_config.get('resolution', (64,64))
            if self.mode == 'joints_only': # If joints_only mode, provide zero image
                img = np.zeros((res[1], res[0], 3), dtype=np.uint8)
            elif rgb is None: # Normal mode, but no image from sim
                img = np.zeros((res[1], res[0], 3), dtype=np.uint8)
            elif rgb.shape[:2] != (res[1], res[0]): # Normal mode, resize if needed
                img = cv2.resize(rgb, res, interpolation=cv2.INTER_AREA)
            else: # Normal mode, use as is
                img = rgb
            obs["image"] = img
        elif self.sensor_config['type'] == 'lidar':
            # In 'joints_only' mode, lidar data is still passed through.
            # If you also want to disable lidar features, add similar logic:
            # if self.mode == 'joints_only':
            #     obs["lidar_points"] = np.zeros((pts,3),dtype=np.float32)
            # else: ... (original lidar processing)
            pts = self.sensor_config.get('max_points',500)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.mode == 'joints_only':
            # Call reset_environment with sequential=True to skip scenery spawning
            self.sim_iface.reset_environment(sequential=True) 
        else:
            # Normal mode: spawn scenery
    ...
    def render(self):
        if self.render_mode in ["human","rgb_array"] and self.sensor_config['type']=='camera':
            img_obs = self._get_observation() # Get current observation
